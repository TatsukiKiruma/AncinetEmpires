...
- 报告末尾输出 2 项当前项目边界探针和 4 项待调查清单，不参与 `--check` 失败判定。边界探针会把支援/突击、致盲/普通反击/反击风暴的当前项目行为转成可对照实机验证项；当前探针结果为：支援会清除目标已行动/已移动标记并把 1 级士兵剩余移动力恢复到 4，同一目标本回合只能被支援一次，未行动目标和更高等级目标不可被支援，等等级目标与同联盟友军可被支援；狼移动 2 格后攻击仍保留 4 点突击后移动力，执行 1 格突击后移动后剩余移动力为 3 且不会再次生成突击后移动；攻击前已致盲的普通单位不普通反击，本次主动攻击刚附加的致盲不取消同一次普通反击，狂战士在 2 格内可触发反击风暴而 3 格不触发，主动击杀目标时目标不反击，被反击击杀的突击单位不保留攻击后移动动作。待调查清单用于后续回填低可信地形和复杂行动顺序等剩余边界。

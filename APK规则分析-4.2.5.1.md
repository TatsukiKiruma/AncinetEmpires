...
1. APK 明确存在初始金币、单位价格等更多可配置规则；项目已接入单位上限、人口上限、可招募列表、收入和等级上限配置，但 APK 默认值仍需反编译或实测确认。
2. 指挥官死亡/复活、后续招募费用等仍需反编译或实测确认。
...
3. 在规则引擎中补齐指挥官死亡/复活、招募费用变化、初始金币/价格覆盖等仍未落地的对战规则。

当前最值得继续落地的代码任务是：价格配置、指挥官死亡/复活/后续招募费用，以及用 APK 实测或反编译结果校准招募 pending 细节。

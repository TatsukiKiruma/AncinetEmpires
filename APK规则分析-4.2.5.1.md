...
3. 指挥官收入默认值仍需确认
   ...
   - 状态：配置层已实现；`apk:skirmish-rule-report` 已门禁当前 SD/SO 默认 `base=0/growth=25` 与 0/1/2 级收入结算，APK 真实默认值仍待实机确认。
...
- 当前覆盖 19 项：开局金币/单位上限/等级上限/模式范围，SD/SO 招募列表，SD 指挥官不在场时可重招募，当前 SD 指挥官费用曲线 `400/400/400` 与 SO 禁用指挥官招募，当前默认 SD/SO 指挥官收入 `base=0/growth=25`，当前默认指挥官死亡后不自动复活且可从城堡重招募，当前默认治疗超上限后不被普通回血、升级或亡灵中毒回血裁剪，当前默认亡灵中毒/墓碑被动回血不突破最大生命，训练 observation 规则暴露，`t30/t31` 回血与清状态差异，`t30/t31` 不占领/不收入/不招募，pending/stacked 招募菜单限制，招募后 pending 来源/扣费/行动标记，投降结算，skirmish 淘汰条件，以及敌军压城堡回合开始扣 50 血并跳过无操作队伍。
- 当前复核结果为 19/19 检查通过；新增检查会门禁当前 SD 指挥官费用曲线 `400/400/400`、SO 禁用指挥官招募、当前默认无自动复活流程、当前默认治疗超上限后的长期非裁剪行为、当前默认亡灵被动回血封顶行为，以及当前默认指挥官收入结算。该工具不重新解包 APK，专门用于防止已实机确认的项目行为和当前训练默认费用/复活/超上限生命/收入假设回退。
...
- `apk:training-report -- --check` 已把该 schema 和 `encodeAction/decodeAction` 全动作类型往返纳入门禁，当前为 14 个模板匹配且往返通过。

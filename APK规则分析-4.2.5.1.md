...
3. APK 资源中的 `data.bin` 已解密并提取单位/地形基础数据，`.aem/.js/.json` 也已确认可用同一 DES key 解密；skirmish `.aem` 头部、队伍 ID、地形矩阵和关键建筑归属已结构化，单位尾部、完整地形 ID 映射和战役特殊规则仍未完全转为项目配置。
...
| 地图/地形 | `data.bin` 84 条地形定义、`.aem` 地图资源、桥为水面文案 | 项目地形标签化，已加入 `bridge` 并按水面处理；`apk_map.ts` 已可读取 skirmish `.aem` 头部、队伍 ID、地形矩阵和已知地形映射 | 84 条 APK tile 到项目 `TerrainId` 的完整映射、单位尾部和完整 `GameState` 导入仍需校准 |
...
| 地形区 | 4 byte/格 | 每格大端整数；`raw >>> 12` 为 APK tile ID，`raw & 0xff` 为归属码，`0..7` 表示队伍，`0xfe/0xff` 当前按中立处理 |

本轮只把头部和地形矩阵作为已确认格式接入；地形区之后的单位/推荐金币/玩家预设/联盟预设尾部仍未完全确认，不能写死为规则。

20 张内置 skirmish 地图的头部和关键地形归属如下。`N` 表示中立/无归属；城堡/城镇统计只统计 APK tile `t37/t36`。

| 地图 | 尺寸 | 队伍 | 作者 | 地形数 | 城堡归属 | 城镇归属 | 损坏城镇 | 桥 | 地形偏移 |
| --- | --- | --- | --- | ---: | --- | --- | ---: | ---: | ---: |
| `(2) Crossed swords.aem` | 11x18 | 0/1 | youxing | 198 | 0:1,1:1,N:2 | 0:2,1:4,N:4 | 0 | 0 | 41 |
| `(2) Duel.aem` | 13x13 | 0/1 | youxing | 169 | 0:1,1:1 | 1:1,N:4 | 1 | 0 | 41 |
| `(2) Icy Paths.aem` | 15x15 | 0/1 | - | 225 | 0:1,1:1,N:2 | 0:2,1:2,N:6 | 0 | 0 | 32 |
| `(2) Liberty Port.aem` | 13x13 | 0/1 | - | 169 | 0:1,1:1,N:1 | N:9 | 0 | 0 | 32 |
| `(2) Mourningstar.aem` | 11x11 | 0/1 | youxing | 121 | 0:1,1:1 | 0:1,1:2,N:5 | 2 | 0 | 41 |
| `(2) Peak Island.aem` | 15x15 | 0/1 | youxing | 225 | 0:1,1:1 | 1:2,N:6 | 2 | 0 | 41 |
| `(2) Swamplands.aem` | 10x10 | 0/1 | youxing | 100 | N:2 | 1:2,N:11 | 1 | 0 | 41 |
| `(2) The Crossing.aem` | 15x10 | 0/1 | youxing | 150 | 0:1,1:1,N:2 | 1:3,N:3 | 0 | 0 | 41 |
| `(3) Frozen fields.aem` | 15x12 | 0/1/2 | - | 180 | 0:1,1:1,2:1 | N:6 | 3 | 0 | 36 |
| `(3) Glu.aem` | 13x19 | 0/1/2 | - | 247 | 0:1,1:1,2:1,N:1 | N:6 | 0 | 0 | 36 |
| `(3) Midway.aem` | 17x12 | 0/1/2 | - | 204 | 0:1,1:1,2:1,N:3 | N:15 | 0 | 0 | 36 |
| `(3) classic 2.aem` | 17x12 | 0/2/3 | - | 204 | 0:1,2:1,3:1 | 0:2,2:2,3:3,N:2 | 2 | 0 | 36 |
| `(4) Critical mass.aem` | 15x15 | 0/1/2/3 | youxing | 225 | 0:1,1:1,2:1,3:1 | 0:2,1:2,2:2,3:2,N:12 | 0 | 0 | 49 |
| `(4) Crossroads.aem` | 11x19 | 0/1/2/3 | youxing | 209 | 0:1,1:1,2:1,3:1 | 0:1,1:1,2:1,3:1 | 4 | 0 | 49 |
| `(4) Shadowlands.aem` | 19x19 | 0/1/2/3 | youxing | 361 | 0:1,1:1,2:1,3:1,N:6 | N:12 | 2 | 0 | 49 |
| `(4) Solitude.aem` | 15x15 | 0/1/2/3 | youxing | 225 | 0:1,1:1,2:1,3:1 | N:12 | 0 | 0 | 49 |
| `(4) The Crucible.aem` | 19x19 | 0/1/2/3 | youxing | 361 | 0:1,1:1,2:1,3:1,N:4 | N:12 | 0 | 0 | 49 |
| `(4) Waterways.aem` | 15x15 | 0/1/2/3 | youxing | 225 | 0:1,1:1,2:1,3:1 | N:8 | 6 | 0 | 49 |
| `(4) Winterstorm.aem` | 13x13 | 0/1/2/3 | youxing | 169 | 0:1,1:1,2:1,3:1 | N:12 | 0 | 0 | 49 |
| `(4) classic 1.aem` | 16x15 | 0/1/2/3 | youxing | 240 | 0:1,1:1,2:1,3:1 | 0:2,1:2,2:2,3:2,N:8 | 0 | 0 | 49 |
...
- 新增 `src/game/apk_map.ts`，解析已解密 `.aem` 明文字节中的 magic、宽高、作者、队伍 ID、5 字节地形数量和 4 字节/格的地形矩阵。
- 确认地形格 `raw >>> 12` 为 APK tile ID，`raw & 0xff` 为归属码；`0..7` 映射队伍，`0xfe/0xff` 目前按中立处理。
- 每个地形格会附带已知的项目 `TerrainId` 映射；当前只映射 `t27/t36/t37/t72` 等高可信地形，未确认 tile 保持 `null`。
- APK 分析文档新增第 8.2 节，归档 20 张内置 skirmish 地图的尺寸、队伍、作者、关键建筑归属和地形偏移。
...
- 本轮除文档外，已补齐脚本指定指挥官的基础对战规则，完成 84 条地形基础数据归档，把神庙清除负面状态改为地形标签语义，按 SD/SO skirmish 脚本修正默认队伍摧毁条件，并开始结构化解析 skirmish `.aem` 地图头部与地形矩阵；当前最重要的差距仍是完整 APK tile 到项目地形映射、`.aem` 单位尾部解析和完整 `GameState` 导入。

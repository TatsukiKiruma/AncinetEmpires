...
1. APK 明确存在单位上限、人口占用、可招募单位列表、初始金币、指挥官基础收入/成长等可配置规则；项目当前人口字段存在，但没有完整执行人口/单位上限和关卡级招募列表。
2. 指挥官基础收入、指挥官死亡/复活、后续招募费用等仍需反编译或实测确认。
...
当前项目伤害公式总体方向一致，但缺失或偏差主要在空军对水中目标、鼓舞状态、虚弱数值和治疗突破上限。
...
5. 加入人口/单位上限和可招募列表
   ...
   - 在 `GameState` 或独立 `RuleConfig` 中加入这些配置。
...
1. 引入 `RuleConfig`，支持收入、等级上限、单位上限、可招募列表等场景配置。
2. 解码或反编译 APK 资源，确认真实数值和战役脚本规则。
3. 用 APK 实测或反编译结果校准招募后的 stacked/pending 细节。

当前最值得继续落地的代码任务是：人口/单位上限、可招募单位列表、指挥官基础收入与 `RuleConfig` 场景配置层。

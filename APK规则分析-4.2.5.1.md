...
- `EnvStepResult` 已直接输出 `legalActionCodes`、`fixedActionSpaceDescriptor` 和 `fixedLegalActionIndexes`，让外部训练进程不必重新拼接字符串编码或固定索引；完整 dense 固定 mask 仍通过 `getFixedActionMask()` 按需获取，避免每步默认返回大数组。
- `apk:training-report -- --check` 已把该 schema、`encodeAction/decodeAction` 全动作类型往返、默认训练场景初始与 smoke 过程的 `legalActions/actionMask` 对齐、`EnvStepResult` 动作序列化字段对齐，以及 40 个默认 APK 训练场景的固定动作索引无碰撞纳入门禁；当前为 14 个模板匹配、往返通过、固定动作空间错配 0 个、动作序列化错配 0 个、mask 错配 0 个。

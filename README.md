...
- **固定全局动作空间 (Fixed Global Action Space)**：当前已提供 `getActionSpaceSchema()` 描述可变参数动作编码模板，并由 `apk:training-report -- --check` 门禁 schema、`encodeAction/decodeAction` 往返和动态 `legalActions/actionMask` 对齐；未来为了兼容 PPO 等强化学习框架，仍可能需要设计固定的稀疏动作矩阵编码。

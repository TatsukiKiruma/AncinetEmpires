...
> **样本量声明**：本报告基于 **一次完整运行** 中实际完成重放的样本，
> n = **60** 个 attempted episode（其中 60 PASS）。
> 这是从 1395 个 episode 的分层抽样中、在 30 分钟预算内跑完的部分
> （单 episode 实测约 4158 ms 中位数 / 31793 ms 均值，100 个跑不进预算），
> 因此**仍是小样本**；通过率与成本外推都带有小样本不确定性，且成本外推假设样本的耗时分布与全归档一致。

## 复现命令

```powershell
node --openssl-legacy-provider v12/tools/run-tool.mjs v12/tools/replay_pilot.ts --target 100 --budget-ms 1800000 --seed 20260923 --episode-timeout-ms 600000 --out-dir v12/out/replay
```
...
| 预算截断未跑（budget-skipped，未验证） | 40 |
| **通过率（PASS / attempted）** | **100.00%** |
| 通过率（PASS / 计划抽样 100） | 60.00% |
| 重放步数（所有 attempt 合计） | 292213 |
| 通过 episode 的重放步数 | 292213 |
| 索引阶段耗时（固定 I/O 成本） | 807 ms |
| 墙钟总耗时 | 1908543 ms（31.81 min） |
| 是否触发预算上限 | 是（--budget-ms 1800000） |
...
| `sd-3p-normal-extra-episodes.jsonl` | 7 | 7 | 0 | 0 |
| `sd-4p-normal-extra-episodes.jsonl` | 3 | 3 | 0 | 0 |
| `sd-high-gold-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-high-unit-limit-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-low-gold-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-low-unit-limit-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-medium-advantage-endgame-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-normal-episodes.jsonl` | 6 | 6 | 0 | 0 |
| `sd-small-advantage-endgame-episodes.jsonl` | 6 | 6 | 0 | 0 |
...
| `sd-3p-normal-extra` | 7 | 7 | 0 | 69445 | 328316 |
| `sd-4p-normal-extra` | 3 | 3 | 0 | 31066 | 183490 |
| `sd-high-gold` | 6 | 6 | 0 | 8864 | 26828 |
| `sd-high-unit-limit` | 6 | 6 | 0 | 57422 | 543639 |
| `sd-low-gold` | 6 | 6 | 0 | 44504 | 255515 |
| `sd-low-unit-limit` | 6 | 6 | 0 | 18957 | 67464 |
| `sd-medium-advantage-endgame` | 6 | 6 | 0 | 14851 | 173962 |
| `sd-normal` | 6 | 6 | 0 | 7295 | 18757 |
| `sd-small-advantage-endgame` | 6 | 6 | 0 | 28341 | 281524 |
...
样本量 n = **60** 个 PASS episode（另有 0 THROW、0 TIMEOUT）；
单 episode 成本实测相差很大（见下表 spread ratio），因此均值与中位数两个口径都给出。

| 指标 | 数值 |
| --- | ---: |
| 每 attempt 平均耗时（含失败） | 31793 ms |
| 每 PASS episode **平均**耗时 | 31793 ms |
| 每 PASS episode **中位数**耗时 | 4158 ms |
| 每 PASS episode 耗时 min / max | 212 / 322137 ms |
| min→max 倍数（spread ratio） | 1519.5× |
| 每 PASS episode 步数 min / 中位数 / max | 69 / 1612 / 29728 |
| 每 PASS step 平均耗时 | 6.5281 ms |
| 每 PASS step 中位数耗时 | 3.4 ms |
| 重放吞吐（均值口径） | 153.19 steps/s, 113.2 episodes/hour |
| 重放吞吐（中位数口径） | 294.12 steps/s, 865.8 episodes/hour |
| 一次性索引成本（约 3.3GB 扫描，每轮只付一次） | 807 ms |

### 全归档 1395 episode 成本外推（**投影，不是实测**）

| 口径 | 每 episode 成本 | 全归档 1395 episode |
| --- | ---: | ---: |
| 均值口径（mean basis，上界） | 31793 ms | **12.32 h**（44351282 ms） |
| 中位数口径（median basis，下界） | 4158 ms | **1.61 h**（5800410 ms） |
| **步数普查口径**：归档实测 9219721 步 × 每步成本 | 3.4（中位）/ 6.5281（均值）ms/step | **8.71 – 16.72 h** |
| **区间** | — | **1.61 – 12.32 小时**（episode 口径） |
| **分层步数口径（首选）** | 见下表按长度分桶 | **约 17.62 h**（区间 13.79 – 17.62 h） |

步数普查（**实测，不是外推**）：归档 1395 个 episode 的 `summary.stepCount` 合计 **9219721** 步（均值 6609.1、中位数 2650 步/episode，缺失 0 条，普查时间 2026-09-23T17:15:25.875Z）。
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| 1-999 | 394 | 202469 | 21 | 10827 | 2.4383 / 4.029 | 否 |
| 1000-4999 | 443 | 1013421 | 24 | 56089 | 3.7547 / 3.5 | 否 |
| 5000-14999 | 267 | 2545805 | 7 | 66748 | 6.6185 / 4.2867 | 否 |
| 15000+ | 291 | 5458026 | 8 | 158549 | 7.7504 / 6.3 | 否 |

**分层投影：约 17.62 小时**（桶内均值口径；桶内中位数口径 13.79 小时，区间 13.79 – 17.62 小时）。
这是**首选**的成本口径：它把"归档真实步数分布"（实测）与"同长度段的每步成本"（抽样实测）结合，
只剩每步成本的不确定性；但最大的桶（15000+ 步，占归档 59% 的步数）只有少量抽样 episode 支撑，这一点必须计入不确定性。

为什么用分桶而不是线性模型：对 60 个 PASS episode 拟合 `elapsedMs = 固定成本 + 每步成本 × steps` 得到 截距 **-7067.1 ms**（**负**）、斜率 7.9791 ms/step、R²=0.86。
线性拟合给出**负**截距，说明每步成本随对局变长而上升（超线性），单一直线模型不适用，故采用分桶投影。

外推依据：上界=每 PASS episode 平均 31793.0ms × 1395；下界=中位数 4158ms × 1395（两者都是外推，不是实测）。

外推假设：假设这 60 个已重放 episode 的耗时分布与归档全部 1395 个 episode 一致；由于单 episode 成本实测相差约 1520 倍，小样本下的均值受长对局离群值驱动，因此同时给出中位数口径。
这是基于 60 个重放成功的 episode 的**投影**：若归档中未抽到的 episode 步数分布与样本不同，投影会偏移；
...
| --- | --- | --- | --- | --- | ---: |
| corrupt initialObservationHash | 是 | `initial-observation-hash-mismatch` | `initial-observation-hash-mismatch` | 是 | 9 |
...
- 可核对样本：60 个（有 planId / mapName / seed 的 episode）
- 落在期望 seed 窗口内的：**60 / 60**

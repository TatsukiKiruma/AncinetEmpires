...
- 当前项目规则代码：`src/game`
- 当前项目规则文档：`README.md`、`docs/training/远古帝国AI训练规则整理.md`、`docs/project/远古帝国AI训练项目上下文.md`
...
2. 指挥官死亡计数和 skirmish 重招募费用增长已按实机确认落地；战役复活/失败流程不纳入当前 AI 对战目标。
...
| 战斗数值 | Wiki 文案、`data.bin` 数值和用户实机验证 | 伤害公式、血量比例、空军对水中单位、鼓舞、虚弱、地形之子、近战大师、远程防御、主动治疗超上限与下一己方回合裁剪、亡灵被动回血封顶等已接入 | 致盲/普通反击/反击风暴/死亡边界等复杂顺序仍需实测 |
...
- 2026-06-30/2026-07-01 用户实机确认：空城堡招募后必须先控制新单位，不能控制其它单位或继续招募，但可以选择结束回合或投降；己方指挥官站城堡时，只有招募单位存在可部署格才允许招募，招募后会出现指挥官与新单位堆叠，此时不能结束回合或投降，必须先处理新单位。

仍需确认：

- APK 里“指挥官站城堡招募”之后的纯 UI 展现仍可继续补样例；当前项目已固化训练规则需要的可部署格前置检查、扣费、pending 来源、行动标记和合法动作集合。
- 指挥官重招募费用已按实机确认：开局无指挥官时为 400，阵亡后 500 起并每死一次 +100，且保留等级/经验；当前 data.bin 单位表确认指挥官基础价格字段为 400。
...
当前命令输出确认：`APK/_analysis/unpack/data.bin` envelope magic 和解密后 magic 均为 `365703`，DES key/iv 为 `72 6b 00 00 00 00 46 46`，加密 payload 从 offset 17 开始；解密后地形记录为 84/84，记录长度为 40/40，和 `src/game/apk_terrain.ts` 的项目归档差异为 0。报告会同时输出防御、回血、移动消耗分布、skirmish 映射可信度汇总和官方 skirmish 实际使用地形的训练规则表；当前汇总为 confirmed=4、atlas=73、approximate=7、unmapped=0。训练规则表集中列出每个 APK tile 的用量、项目地形映射、可信度、data.bin 防御/回血/移动数值，以及清状态、占领、收入和招募语义。
...
2026-07-01 起，DEX 字符串表与关键方法签名复核已固化为 `tools/apk_dex_report.ts`，可通过以下命令重复验证：

```bash
npm run apk:dex-report -- --check
```

当前命令输出确认：`APK/_analysis/unpack/classes.dex` 可解析出 26529 个字符串；必要字符串缺失 0，必要方法名缺失 0，关键字符串引用方法 9 个，关键规则方法证据 8 个且缺失期望 0 项；`CheckCommander`、`GetCommander`、`SyncSetCommander`、`SetIncomeCommanderBase`、`SetIncomeCommanderGrowth`、`SetLevelCap`、`SetPrices`、`SyncSetGold`、`SyncSetRecruitUnits`、`SyncSetRecruitUnitsForTeam`、`SyncSetUnitLimit`、`Cannot recruit when stacked!`、`OnUnitRecruited`、`Cannot attack from (`、`Cannot attack in state [`、`Cannot support from (`、`Cannot support in state [`、`AsyncAttack` 和 `SyncSetUnitStatus` 等必要字符串均存在；方法表可解析 `CheckCommander`、`GetCommander`、`SyncSetCommander`、`SetIncomeCommanderBase/Growth`、`SyncSetRecruitUnits*`、`AsyncAttack` 和 `SyncSetUnitStatus` 的关键签名；字符串反查确认 `Cannot attack from/state` 引用到 `Lc/a/b/a/l;.i(int,int)`，`Cannot support from/state` 引用到 `Lc/a/b/a/l;.m(int,int)`，`Cannot recruit when stacked!` 引用到 `Lc/a/b/a/l;.c(int,int,int)`；关键方法字节码引用确认攻击/支援入口读取 `Lc/a/b/a/t/e;.a:I` 且匹配状态字面量 2，招募入口读取 pending 字段 `Lc/a/b/a/t/a;.d:Unit` 且匹配状态字面量 1，并分别调用 `q.a(Unit,int,int)`、`q.h(Unit,int,int)`、`q.b(int,int,int)` 做规则校验；二级方法确认 `q.a(Unit,int,int)` 引用攻击目标/地形/能力校验，`q.i(Unit,Unit)` 引用 `counter_storm` 能力、距离/射程判断和字面量 2，且关键操作顺序位于普通射程校验之前，`q.c(Unit,Unit)` 引用 `poisoner`、`blinder`、状态枚举与状态应用方法，且关键操作顺序为先中毒后致盲；`q.h(Unit,int,int)` 按坐标取目标后委托 `q.n(Unit,Unit)`，`q.n(Unit,Unit)` 引用目标行动状态、已支援标记、等级字段和支援者能力排除校验，且关键操作顺序为先行动/支援状态，再关系/能力排除，最后等级比较；上述三段顺序已作为 `operation-order` 期望进入缺失项统计；构造器字节码确认 `SetIncomeCommanderBase` 对应字段默认 50、`SetIncomeCommanderGrowth` 对应字段默认 25；按 `ReviveCommander/RespawnCommander/CommanderRevive/CommanderRespawn` 匹配的疑似指挥官复活 API 字符串为 0。该报告证明 DEX API 暴露、签名、关键字符串引用、默认规则初始化、关键动作入口调用、反击风暴、攻击附加状态和部分二级规则校验证据，不等同于完整 Java 控制流反编译。
...
| `Lc/a/b/a/o` | `SyncSetRecruitUnits(int[] units) -> void` | 设置通用可招募单位 |
...
| `Lc/a/b/a/l` | `i(int, int)` 引用 `Cannot attack from (` / `Cannot attack in state [`；读取 `Lc/a/b/a/t/e;.a:I`，状态字面量为 2；调用 `Lc/a/b/a/q;.a(Unit,int,int):boolean` 与 `Lc/a/b/a/q;.m(Unit)` | 攻击入口存在动作状态校验、来源/目标合法性校验和攻击效果查询 |
| `Lc/a/b/a/l` | `m(int, int)` 引用 `Cannot support from (` / `Cannot support in state [`；读取 `Lc/a/b/a/t/e;.a:I`，状态字面量为 2；调用 `Lc/a/b/a/q;.h(Unit,int,int):boolean` | 支援入口存在动作状态校验和目标合法性校验 |
| `Lc/a/b/a/l` | `c(int, int, int)` 引用 `Cannot recruit when stacked!`；读取 `Lc/a/b/a/t/a;.d:Unit`，状态字面量为 1；调用 `Lc/a/b/a/q;.b(int,int,int):boolean` | 招募入口存在 stacked/pending 校验、招募状态校验和招募合法性校验 |
| `Lc/a/b/a/q` | `a(Unit, int, int) -> boolean` 引用地图占用、地形 raw tile、能力枚举 `t/g.f`，并调用 `e(Unit,int,int)`、`j(Unit,Unit)`、`a(Unit,Ability)` | 攻击目标/射程/地形或能力合法性在规则层集中校验 |
| `Lc/a/b/a/q` | `h(Unit, int, int) -> boolean` 调用 `k(int,int)` 取目标单位，再调用 `n(Unit,Unit)` | 支援坐标校验会先解析目标单位，再进入目标规则校验 |
| `Lc/a/b/a/q` | `n(Unit, Unit) -> boolean` 读取 `Unit.i`、`Unit.j`、`Unit.n`，引用能力枚举 `t/g.y`，并调用 `g(Unit,Unit)`、`a(Unit,Ability)` | 支援目标合法性包含目标行动/已支援状态、等级比较、友军关系和支援者能力排除校验 |
...
当前 DEX 字符串报告和方法表没有发现通用的 `ReviveCommander`、`RespawnCommander` 一类 API；此前只在战役剧情文本中见到 `revive Saeth`。因此只能确认 skirmish 的城堡重招募流程，不能把战役复活/剧情失败流程推导为通用对战规则。
...
4. 修正治疗上限
   ...
   - 2026-07-01 用户实机确认：主动治疗可超过最大生命；若下一己方回合开始时生命仍高于最大生命，会先裁剪到最大生命，再进行普通回合开始回血。普通封顶回复、升级回满血、亡灵中毒/墓碑回血不会把既有超上限生命压回最大生命。
   - 状态：主动治疗超上限、己方回合开始裁剪、亡灵中毒/墓碑回血封顶和普通封顶回复保留既有超上限均已完成。
...
6. 完善指挥官经济与死亡规则
   ...
   - 状态：基础收入/成长收入、死亡计数、skirmish 重招募价格增长已完成；战役复活/剧情失败流程不纳入当前 AI 对战目标。
...
| 指挥官死亡/复活完整规则 | 中 | skirmish 城堡重招募已实测并门禁；当前 APK 文本和方法表未发现通用复活 API，战役流程不纳入当前目标 |
...
当前最值得继续落地的规则任务是：继续扩大脚本配置应用面，并用 APK 实测或更完整反编译结果校准支援/突击顺序、致盲/普通反击/反击风暴的精确执行顺序和低可信地形。
...
- `RuleConfig.commanderRecruitBaseCost/commanderRecruitCostGrowth` 支持在配置开启后按死亡次数递增指挥官重招募费用；skirmish SD 规则按实机确认使用 400/500/600... 费用曲线，SO 禁用指挥官招募。
...
- 未发现通用指挥官复活/重生 API；skirmish 重招募流程已按实机确认，战役复活/剧情失败流程不纳入当前目标。
...
- 当前覆盖 28 项：开局金币/单位上限/等级上限/模式范围，SD/SO 招募列表，SD/SO 默认招募费用和人口占用，SD 指挥官不在场时可重招募，SD 指挥官费用曲线 `400/500/600` 与 SO 禁用指挥官招募，DEX 默认规则数据确认的 SD/SO 指挥官收入 `base=50/growth=25`，DEX 支援目标/反击风暴/攻击附加状态关键顺序证据，OPENCODE 反编译确认的普通反击距离 1 与反击附加状态，APK 语言表能力/状态规则 28 项复核汇总，APK 脚本 manifest 与 7 项脚本规则/状态配置应用检查汇总，APK data.bin 单位/能力/状态/地形 manifest 摘要，skirmish 指挥官死亡后不自动复活且重招募继承等级/经验，主动治疗超上限后下一己方回合开始先裁剪到最大生命，升级不裁剪既有超上限生命，亡灵中毒/墓碑被动回血不突破最大生命且不压低既有超上限生命，默认 20 张 skirmish 训练地图和 40 个训练场景只包含已验证 approximate `t30/t31` 且不含未实测 approximate/t80/t81/t82/t83，训练 observation 规则暴露，`t30/t31` 回血与清状态差异，`t30/t31` 不占领/不收入/不招募，APK 地形防御参与战斗且飞行单位不吃地形防御，pending/stacked 招募菜单限制，招募后 pending 来源/扣费/行动标记，投降结算，skirmish 淘汰条件，以及敌军压城堡回合开始扣 50 血并跳过无操作队伍。
- 当前复核结果为 28/28 检查通过；新增检查会门禁 SD/SO 默认招募费用和人口占用、SD 指挥官费用曲线 `400/500/600`、SO 禁用指挥官招募、skirmish 无自动复活且重招募继承等级/经验、治疗超上限后的回合开始裁剪、亡灵被动回血封顶、当前训练默认指挥官收入结算和静态来源证据、默认训练地图和训练场景低可信地形风险隔离、DEX 三段关键操作顺序证据、OPENCODE 反编译确认的反击射程/状态附加语义、APK 语言表能力/状态规则 28 项复核汇总、APK 脚本 manifest 与 7 项应用检查汇总、APK data.bin `defenseBonus` 进入战斗结算，以及已归档 data.bin 的 21 个单位 ID、26 个能力 ID、4 个状态 ID、84 条地形记录、40 字节地形记录长度和 skirmish 地形映射可信度分布。该工具现在会读取 `APK/_analysis/unpack/classes.dex`、同步复用 `apk:language-rule-report`，并汇总脚本 manifest/应用检查和 data.bin manifest 摘要；完整 DES 解密仍由 `apk:script-report -- --check`、`apk:unit-report -- --check`、`apk:terrain-report -- --check` 单独门禁，避免普通单测依赖 legacy DES provider。
- 报告末尾输出 3 项当前项目边界探针和 4 项待调查清单，不参与 `--check` 失败判定。边界探针会把支援/突击、致盲/普通反击/反击风暴、鼓舞/虚弱伤害组合的当前项目行为转成可对照实机验证项；当前探针结果为：支援会清除目标已行动/已移动标记并把 1 级士兵剩余移动力恢复到 4，同一目标本回合只能被支援一次，未行动目标和更高等级目标不可被支援，等等级目标与同联盟友军可被支援；狼移动 2 格后攻击仍保留 4 点突击后移动力，执行 1 格突击后移动后剩余移动力为 3 且不会再次生成突击后移动；攻击前已致盲的普通单位不普通反击，本次主动攻击刚附加的致盲不取消同一次普通反击，普通远程单位在 2 格不会普通反击，反击者带投毒时会给原攻击者附加中毒，狂战士在 2 格内可触发反击风暴而 3 格不触发，主动击杀目标时目标不反击，被反击击杀的突击单位不保留攻击后移动动作；当前项目中士兵近战打士兵为 `50/60/60/70`（普通/鼓舞/虚弱防守/鼓舞打虚弱防守），弓箭手 2 格打士兵为 `40/45/45/50`。待调查清单用于后续回填低可信地形和复杂行动顺序等剩余边界。
...
- `tools/apk_training_report.ts` / `npm run apk:training-report -- --check` 会解密默认 20 张官方 skirmish 地图，生成 SD/SO 共 40 个训练场景并逐一创建 `AncientEmpiresEnv`。当前复核结果为 40/40 manifest 匹配、40/40 metadata 匹配，含未实测 approximate 的场景 0 个且涉及 APK tile ID 为无，模式规则错配 0 个，指挥官重招募费用错配 0 个，Observation 招募经济错配 0 个，Observation APK 地形/单位证据字段错配 0 个，固定动作空间错配 0 个，初始与 smoke 过程 actionMask 错配 0 个，动作序列化错配 0 个，动作接口 schema 14 个模板匹配且 `encodeAction/decodeAction` 往返通过，初始合法动作数均大于 0，且默认每场景执行 4 个合法动作 smoke test 无失败；报告表格会列出每个场景的 approximate ID 和未实测 approximate ID，`--include-approximate` 仍可用于未来放行未实测 approximate 地图。该报告会显示并门禁 SD 默认指挥官费用曲线 `400/500/600`、SO 禁用指挥官招募、SD/SO 可招募单位列表、每个玩家 `players[].recruitCosts` 中的 APK 默认招募费用、训练 Observation 中的 APK tile 数值、映射证据和单位静态数值字段、训练动作编码面、固定动作索引、`EnvStepResult` 动作序列化字段和 `legalActionEntries` 条目，以及动态 `legalActions/actionMask` 对齐。
...
- 全 APK assets AEM 使用量已由 `npm run apk:map-report -- --check` 门禁：`t30=30`、`t31=16`、`t81=9`、`t82=8`、`t83=6`、`t80=0`；其中 `t81/t82/t83` 只出现在 AEIII 战役地图，不影响 20 张官方 skirmish 地图。报告会输出资源路径和坐标样例：`t81/t82` 均只出现在 `assets/mods/AEIII/s4.aem`，`t83` 出现在 `assets/mods/AEIII/s4.aem` 与 `assets/mods/AEIII/s6.aem`。该统计对 `assets/mods/AEII/s5.aem`、`assets/mods/AEII/s7.aem`、`assets/mods/AEIII/s1.aem` 使用只解析地形段的路径，因为这 3 张战役图单位段不符合当前 skirmish 单位解析格式。
...
- 当前 `npm run apk:map-report -- --check` 结果：APK SHA256 匹配，20/20 地图 manifest 匹配，尾部模板 `zero_suffix_58=20`，非预期尾部 0，4 张地图含 approximate tile，0 张地图含 unmapped tile；报告末尾直接输出 `t30/t31` 人工验证坐标、当前项目语义 checklist，并显示 `t30/t31` 的实机确认状态。该命令还会扫描全 `assets` 下 45 张 `.aem` 的地形段并门禁低可信 tile 使用范围：`t30=30`、`t31=16`、`t80=0`、`t81=9`、`t82=8`、`t83=6`，其中 `t81/t82/t83` 的 skirmish 格子数均为 0，并输出资源路径与坐标样例。
...
- 当前 `npm run apk:dex-report -- --check` 结果：26529 个字符串可解析，必要字符串缺失为 0，必要方法名缺失为 0，关键字符串引用方法为 9，关键规则方法证据为 8 个且缺失期望 0 项，默认指挥官收入为 `base=50/growth=25`，攻击动作关键词命中 4，支援动作命中 2，状态 Stage 命中 4，`revive` 关键词分组命中 0，`ReviveCommander/RespawnCommander/CommanderRevive/CommanderRespawn` 候选为 0。新增证据覆盖 `q.i(Unit,Unit)` 的 `counter_storm`/距离字面量 2 校验，以及 `q.c(Unit,Unit)` 的投毒/致盲状态应用引用；操作顺序表进一步显示 `q.n` 先检查行动/支援状态，再做关系/能力排除和等级比较，`q.i` 在普通射程校验前处理 `counter_storm` 与距离 2，`q.c` 先处理中毒后处理致盲，且这三段顺序已作为 `operation-order` 期望进入缺失项统计。
- 当前可重复解析的关键方法签名包括：`AsyncAttack(int, int, int)`、`AsyncAttack(int, int, int, int)`、`CheckCommander(Unit)`、`CheckCommander(Unit, int)`、`GetCommander(int)`、`SetIncomeCommanderBase(int)`、`SetIncomeCommanderGrowth(int)`、`SyncSetCommander(int, int)`、`SyncSetRecruitUnits(int[])`、`SyncSetRecruitUnitsForTeam(int, int[])`、`SyncSetUnitStatus(int, int, int, int, boolean)`；关键字符串引用反查还确认攻击、支援、招募 stacked 和状态设置错误文本都能定位到具体方法；关键动作方法字节码引用确认攻击/支援状态为 2、招募状态为 1，且三者分别调用 `q.a(Unit,int,int)`、`q.h(Unit,int,int)`、`q.b(int,int,int)` 做规则合法性判断；二级方法引用进一步确认 `q.h` 委托 `q.n`，而 `q.n` 包含行动/支援状态、等级和支援者能力排除相关字段。
...
- 工具会逐项对比真实 `data.bin` 解析结果和 `APK_TERRAIN_CONFIGS` 归档，并输出防御、回血、移动消耗分布、skirmish 映射可信度汇总，以及官方 skirmish 实际使用地形的训练规则表。
- 当前 `npm run apk:terrain-report -- --check` 结果：84/84 地形记录匹配，项目归档差异为 0；防御分布为 0/5/10/15/20，回血分布为 0/3/20，移动分布为 1/2/3/16777215；训练规则表包含 `t30/t31` 已验证语义，不包含 `t80/t81/t82/t83`。

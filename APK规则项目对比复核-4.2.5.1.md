...
当前命令输出确认：28/28 项检查通过，覆盖遭遇战开局设置范围、SD/SO 招募列表、SD/SO 默认招募费用和人口占用、SD 指挥官不在场时可重招募、SD 指挥官费用曲线 `400/500/600` 与 SO 禁用指挥官招募、DEX 默认规则数据确认的 SD/SO 指挥官收入 `base=50/growth=25`、DEX 支援目标/反击风暴/攻击附加状态关键顺序证据、OPENCODE 普通反击距离 1 和反击附加状态检查、APK 语言表能力/状态规则 28 项复核汇总、APK 脚本 manifest 与 7 项脚本规则/状态配置应用检查汇总、APK data.bin 单位/能力/状态/地形 manifest 摘要、skirmish 指挥官死亡后不自动复活且重招募继承等级/经验、主动治疗超上限后下一己方回合开始先裁剪到最大生命、升级不裁剪既有超上限生命、亡灵中毒/墓碑被动回血不突破最大生命且不压低既有超上限生命、默认 20 张 skirmish 训练地图和 40 个训练场景只包含已验证 approximate `t30/t31` 且不含未实测 approximate/t80/t81/t82/t83、开局设置对训练状态的约束、训练 observation 暴露的规则/费用/指挥官/pending 状态、`t30/t31` 回血与清状态差异、`t30/t31` 不占领/不收入/不招募、APK 地形防御参与战斗且飞行单位不吃地形防御、pending/stacked 招募菜单限制、招募后 pending 来源/扣费/行动标记、投降结算、skirmish 淘汰条件和敌军压己方城堡回合开始扣 50 血。报告末尾还输出 3 项当前项目边界探针和 4 项待调查清单，不参与 `--check` 失败判定；探针用于把支援/突击、致盲/反击风暴、鼓舞/虚弱伤害组合的当前项目行为转成可对照实机验证项。
...
当前命令输出确认：APK SHA256 匹配 `51B00185F300DD8899284AA91986AEE9A1CC73FA012262A0D9EEBC97FAD1AA7B`，20 张 `assets/maps/*.aem` 全部可解密解析，20/20 与 `src/game/apk_manifest.ts` 清单匹配，unmapped tile 数为 0。报告同时列出 4 张含 approximate tile 的地图和 `t30/t31` 人工验证坐标；还会扫描全 `assets` 下 45 张 `.aem` 的地形段并门禁低可信 tile 使用范围：`t30=30`、`t31=16`、`t80=0`、`t81=9`、`t82=8`、`t83=6`，其中 `t81/t82/t83` 的 skirmish 格子数均为 0。全 AEM 表会同时输出资源路径和坐标样例，当前 `t81/t82` 均只出现在 `assets/mods/AEIII/s4.aem`，`t83` 出现在 `assets/mods/AEIII/s4.aem` 与 `assets/mods/AEIII/s6.aem`。
...
| 9 | 指挥官 / Commander | `commander` | 已实现；SD skirmish 中无本方存活指挥官时可招募 |
...
- 治疗师主动治疗已允许持续突破最大血量；用户 2026-07-01 实机确认下一己方回合开始时若仍超上限会先裁剪到最大生命。普通封顶回血和升级回满血不会继续突破上限，也不会把既有超上限生命压回最大生命。
- 亡灵墓碑/中毒回血已按用户 2026-07-01 实机确认封顶到最大生命；项目封顶回复不会压低既有超上限生命。地形和光环等普通回血仍按封顶回复处理。
- 反击风暴和攻击附加状态已有 DEX 方法与关键操作顺序证据；OPENCODE 反编译确认普通反击必须距离 1 且在防守方射程内，反击命中后同样附加中毒/致盲状态，项目已落地并进入报告门禁。死亡边界与突击移动在复杂状态下的官方精确顺序仍需更多实测。当前项目探针结果：攻击前已致盲的普通单位不会普通反击；本次主动攻击刚附加的致盲不取消同一次普通反击；普通远程单位 2 格不会普通反击；反击者带投毒时会给原攻击者附加中毒；带 `counter_storm` 的狂战士在 2 格内即使被致盲也会反击，3 格不反击；主动攻击直接击杀目标时目标不会反击；突击单位若被反击击杀，会被清除并生成墓碑，不再保留攻击后移动动作；支援会清除目标已行动/已移动标记并恢复有效剩余移动力，同一目标被支援并再次行动后，本回合第二名支援者不能再次支援，未行动目标和更高等级目标不可被支援，等等级目标与同联盟友军可被支援；狼移动 2 格后攻击，攻击后剩余移动力为 4，最远突击后移动距离为 4，执行 1 格突击后移动后剩余移动力为 3 且不再生成突击后移动；鼓舞/虚弱组合探针中，士兵近战打士兵为 `50/60/60/70`（普通/鼓舞/虚弱防守/鼓舞打虚弱防守），弓箭手 2 格打士兵为 `40/45/45/50`。
...
- pending 存在时只生成该单位动作，且不生成招募。2026-06-30/2026-07-01 用户实机确认：空城堡招募 pending 仍可结束回合/投降；指挥官站城堡只有存在可部署格才允许招募，产生的堆叠招募 pending 不能结束回合或投降。
- `RuleConfig.allowSurrender` 已支持结构化 `surrender` 动作；普通规则默认关闭，`createApkSkirmishGameState` 的 SD/SO 规则默认开启。
- 执行 `surrender` 会让当前队伍失活、删除该队全部单位、把该队占领建筑变为无主，并复用既有联盟胜负结算；内置训练 AI 不会把投降当作普通非结束动作随机优先选择。
- `AncientEmpiresEnv.getObservation()` 已输出 `pendingUnitId` 和单位级 `isPending/apkPendingRecruitSource`，训练侧可直接观察 stacked 状态。
...
- 20 张图的 58 字节尾部全部为 `zero_suffix_58`，不能据此推导联盟、玩家颜色或阵营预设；`npm run apk:map-report -- --check` 已把非 `zero_suffix_58` 尾部作为失败条件，避免后续地图样本漂移被默默接受。
...
- `GameState.metadata` 可保留 APK 版本、APK SHA256、APK 内资源路径、地图名、skirmish 模式、遭遇战开局设置范围、推荐金币和尾部模板。
...
当前命令输出确认：27/27 个 `assets/mods/**/*.js` 可用 `DES/CBC/PKCS7` 和 key/iv `72 6b 00 00 00 00 46 46` 解密；`Stage.*` 与 `rule.SetIncome*` API 计数和 `APK_SCRIPT_API_CALL_COUNTS` 完全一致；可安全提取的 `APK_SCRIPT_LITERAL_RULE_CONFIGS` 与 `APK_SCRIPT_LITERAL_STAGE_STATE_CONFIGS` 均无差异；应用检查 7/7 通过，覆盖脚本招募、联盟、禁用队伍、收入覆盖、移动覆盖和状态覆盖进入 Observation，并验证 `assets/mods/AEIII/s6.js` 的禁用队伍不会生成合法动作且回合轮转会跳过该队伍；其中 `assets/mods/AEIII/s4.js` 的 `rule.SetIncomeVillage(100)` 已验证会让只拥有一个村庄的队伍在回合开始金币从 500 增至 600。
...
训练接口层还新增 `getActionSpaceSchema()`，返回当前结构化动作的字符串编码模板，覆盖移动、突击后移动、攻击、治疗、支援、召唤、招募、占领、修理、摧毁城镇、待机、投降和结束回合。固定稀疏动作空间由 `getFixedActionSpaceDescriptor()`、`encodeFixedActionIndex()`、`getFixedLegalActionIndexes()`、`getFixedActionMask()` 和 `stepFixedAction()` 提供，按地图宽高、源格、目标格、招募兵种和全局动作分块计算；`EnvStepResult` 会直接输出 `legalActionCodes`、`legalActionEntries`、`fixedActionSpaceDescriptor` 和 `fixedLegalActionIndexes`，其中 `legalActionEntries[]` 聚合结构化动作、字符串编码、动态 mask 位和固定索引，完整 dense 固定 mask 仍通过 `getFixedActionMask()` 按需获取。实际动作可用性仍由每个局面的 `legalActions/actionMask` 决定。`apk:training-report -- --check` 已门禁默认 40 个训练场景的初始与 smoke 过程 mask 长度和值均与合法动作列表一致，并确认动作序列化字段和条目对齐、固定动作索引无碰撞。
...
1. 维护 skirmish 高频 tile 语义，并继续校准非默认低可信 APK tile
   ...
   - `apk:map-report -- --check` 已门禁全 45 张 assets AEM 的低可信 tile 分布：`t80=0`，`t81/t82/t83` 的 skirmish 格子数均为 0，仅出现在非 skirmish 战役资源中；报告会输出这些非 skirmish tile 的资源路径和坐标样例，供实机定位。

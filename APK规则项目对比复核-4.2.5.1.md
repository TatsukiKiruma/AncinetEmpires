...
- 反击风暴和攻击附加状态已有 DEX 方法与关键操作顺序证据，但致盲、普通反击、反击风暴、死亡边界与突击移动在复杂状态下的官方精确顺序仍需更多实测。当前项目探针结果：普通单位被致盲后不会普通反击；带 `counter_storm` 的狂战士在 2 格内即使被致盲也会反击，3 格不反击；同一目标被支援并再次行动后，本回合第二名支援者不能再次支援；狼移动 2 格后攻击，攻击后剩余移动力为 4，最远突击后移动距离为 4。

...
- 治疗师主动治疗已允许持续突破最大血量，但 APK 是否在后续回合或回合开始阶段裁剪回最大血量未知。
...
该 manifest 仍不是完整脚本执行器；含动态参数的配置和剧情触发仍需独立场景层处理。
...
2. 系统归档脚本配置
   - 已归档 API 计数、可直接提取的字面量配置分布，以及 26 个脚本的逐脚本字面量配置表。
   - 下一步输出逐关卡配置表，处理动态参数，避免把战役差异硬编码到引擎。

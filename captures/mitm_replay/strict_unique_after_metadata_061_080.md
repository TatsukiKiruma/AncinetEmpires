...
| 分类 | 数量 | 首个失败 |
| --- | ---: | --- |
| strict 移动可达性缺口 (`strict_move_reachability_gap`) | 2 | game_get_20260704_132008_065.bin#55 STANDBY move:u_13:4,6<br>game_get_20260704_132050_078.bin#51 ATTACK move:u_11:10,5 |
...
| game_get_20260704_132008_065.bin | 1269141a5313 | 13x13, P[4,1,0,5,2,3] | 1182 | 失败 | 55/1182 | 非法动作：当前玩家无法执行该动作 |
...
| game_get_20260704_132050_078.bin | 72d58b04ba9f | 13x13, P[4,1,0,5,2,3] | 250 | 失败 | 51/250 | 非法动作：当前玩家无法执行该动作 |
...
- 地图：21x21，scenario=SD，campaignLike=false，玩家=[0,1,2,3,4,5]，初始单位=6，推荐金币=300
...
- 地图：25x21，scenario=SD，campaignLike=false，玩家=[0,1,2,3,4,5]，初始单位=6，推荐金币=-1
...
- 地图：11x19，scenario=SD，campaignLike=false，玩家=[0,1,2,3,5]，初始单位=5，推荐金币=-1
...
- 地图：13x13，scenario=SR，campaignLike=false，玩家=[4,1,0,5,2,3]，初始单位=10，推荐金币=250
...
- 校验进度：已执行 55 / 1182 条 APK 记录，展开项目动作 83 个
- 终局状态：turn=2，currentPlayer=3，winner=-
- 首个问题：非法动作：当前玩家无法执行该动作
- 分歧分类：strict 移动可达性缺口（strict_move_reachability_gap）：APK 记录要求移动到 (4,6)；项目没有生成对应合法移动动作；优先核实 APK 运行时脚本、SyncOverrideMov 或前序状态，而不是直接修改全局地形成本；terrainId=2, owner=-, apkTerrainId=63, apkKind=0, apkIsLand=false, apkMoveCost=3
- APK 记录：event=STANDBY，source=(3,3)，move=(4,6)，target=-，recruitId=-1
- 问题位置：record=55, turn=2, currentPlayer=3, legalActions=24
- 项目动作：`move:u_13:4,6`
- 失败诊断：actor=u_13/soldier/P3@(3,3) hp=100/100 lv=0 exp=0 range=1-1 status=none；target=-；move占位=-；move地形=terrain=2 owner=- apkTerrainId=63 apkKind=0 apkIsLand=false apkMoveCost=3；sourceTargetDist=-；moveTargetDist=-
...
- 地图：17x12，scenario=SD，campaignLike=false，玩家=[0,2,3,4]，初始单位=4，推荐金币=300
...
- 地图：9x13，scenario=SD，campaignLike=false，玩家=[0,1,2,3,5]，初始单位=5，推荐金币=200
...
- 地图：21x25，scenario=SD，campaignLike=false，玩家=[0,1,2,3,4,5]，初始单位=0，推荐金币=-1
- 项目开局：currentPlayer=0，activeTeams=[0,1,2,3,4,5]，initialGold=700，unitLimit=100，levelCap=9
- 本局价格覆盖：soldier=150, archer=250, water_elemental=300, witch=400, elf=500, wolf=600, golem=600, catapult=800, dragon=1000, commander=400, paladin=400, berserker=500, ghost=200, dark_mage=300, wolf_archer=800, ice_elemental=600, slime=250, mermaid=200, druid=600
...
- 地图：25x25，scenario=SD，campaignLike=false，玩家=[0,1,2,3,4,5]，初始单位=6，推荐金币=1300
- 项目开局：currentPlayer=0，activeTeams=[0,1,2,3,4,5]，initialGold=700，unitLimit=100，levelCap=9
- 本局价格覆盖：soldier=150, archer=250, water_elemental=300, witch=400, elf=500, wolf=600, golem=600, catapult=800, dragon=1000, commander=400, paladin=400, berserker=500, ghost=200, dark_mage=300, wolf_archer=800, ice_elemental=600, slime=250, mermaid=200, druid=600
...
- 地图：13x13，scenario=SD，campaignLike=false，玩家=[4,1,0,5,2,3]，初始单位=10，推荐金币=250
...
- 包诊断：topFields=[g]，topRemaining=0，onlyG=true，replayEOF=true，currentStep=0，replaySplitIndex=0，room=PUBLIC/CLOSED
- 校验进度：已执行 51 / 250 条 APK 记录，展开项目动作 76 个
- 终局状态：turn=2，currentPlayer=1，winner=-
- 首个问题：非法动作：当前玩家无法执行该动作
- 分歧分类：strict 移动可达性缺口（strict_move_reachability_gap）：APK 记录要求移动到 (10,5)；项目没有生成对应合法移动动作；优先核实 APK 运行时脚本、SyncOverrideMov 或前序状态，而不是直接修改全局地形成本；terrainId=3, owner=-, apkTerrainId=17, apkKind=1, apkIsLand=true, apkMoveCost=3
- APK 记录：event=ATTACK，source=(11,7)，move=(10,5)，target=(10,4)，recruitId=-1
- 问题位置：record=51, turn=2, currentPlayer=1, legalActions=45
- 项目动作：`move:u_11:10,5`
- 失败诊断：actor=u_11/soldier/P1@(11,7) hp=100/100 lv=0 exp=0 range=1-1 status=none；target=apk_remote_u1/ghost/P4@(10,4) hp=100/100 lv=0 exp=0 range=1-1 status=none；move占位=-；move地形=terrain=3 owner=- apkTerrainId=17 apkKind=1 apkIsLand=true apkMoveCost=3；sourceTargetDist=4；moveTargetDist=1
- 引擎信息：非法动作：当前玩家无法执行该动作
...
- 地图：15x15，scenario=SD，campaignLike=false，玩家=[0,1,2,3,4,5]，初始单位=6，推荐金币=300

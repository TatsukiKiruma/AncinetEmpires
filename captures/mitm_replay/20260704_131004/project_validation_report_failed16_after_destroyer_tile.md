...
- 生成时间：2026-07-04T14:32:08.307Z
- 输入目录：`C:\code\AncinetEmpires\captures\mitm_replay`
- 规则模式：SD
- 抽样文件上限：全部
- 招募歧义处理：允许选第一个合法部署点继续
- 地形处理：未映射地形使用道路兜底并记录
- 文件数：16，唯一校验：16，重复跳过：0，通过：5，失败：11，解析错误：0
...
| game_get_20260704_132013_067.bin | e84f62fe505c | 9x13, P[0,1,2,3,5] | 721 | 失败 | 147/721 | 找不到召唤目标墓碑: (4, 2) |
...
- 校验进度：已执行 147 / 721 条 APK 记录，展开项目动作 220 个
- 终局状态：turn=4，currentPlayer=5，winner=-
- 首个问题：找不到召唤目标墓碑: (4, 2)
- APK 记录：event=SUMMON，source=(6,2)，move=(6,2)，target=(4,2)，recruitId=-1
- 问题位置：record=147, turn=4, currentPlayer=5, legalActions=82
- 项目动作：`-`
- 引擎信息：-

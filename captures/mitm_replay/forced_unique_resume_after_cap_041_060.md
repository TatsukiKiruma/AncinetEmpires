...
| 分类 | 数量 | 首个失败 |
| --- | ---: | --- |
| 未分类 strict 分歧 (`unknown`) | 1 | game_get_20260704_131516_044.bin#3 HEAL - |
...
| game_get_20260704_131516_044.bin | 6c0f7adb9593 | 21x25, P[0,1,2,3,4,5] | 13 | 失败 | 3/13 | 找不到治疗目标单位: (10, 13) |
...
- 项目开局：currentPlayer=4，activeTeams=[0,1,2,3,4,5]，initialGold=2000，unitLimit=100，levelCap=9
- 本局价格覆盖：soldier=150, archer=250, water_elemental=300, witch=400, elf=500, wolf=600, golem=600, catapult=800, dragon=1000, commander=400, paladin=400, berserker=500, ghost=200, dark_mage=300, wolf_archer=800, ice_elemental=600, slime=250, mermaid=200, druid=600
- 动作：13 条，remaining=966，事件=ATTACK=11, HEAL=2
- 包诊断：topFields=[g]，topRemaining=0，onlyG=true，replayEOF=false，currentStep=15，replaySplitIndex=-1，room=PUBLIC/CLOSED
- 校验进度：已执行 3 / 13 条 APK 记录，展开项目动作 0 个
- 终局状态：turn=1，currentPlayer=4，winner=-
- 首个问题：找不到治疗目标单位: (10, 13)
- 分歧分类：未分类 strict 分歧（unknown）：需要继续按首个 record 查看回放记录和 APK 反编译证据。
- APK 记录：event=HEAL，source=(10,15)，move=(10,14)，target=(10,13)，recruitId=-1
- 问题位置：record=3, turn=1, currentPlayer=4, legalActions=741
- 项目动作：`-`
- 失败诊断：actor=apk_remote_u8/slime/P4@(10,15) hp=100/100 lv=0 exp=0 range=1-1 status=none；target=-；move占位=apk_remote_u7/wolf/P4@(10,14) hp=100/100 lv=0 exp=0 range=1-1 status=none；move地形=terrain=17 owner=- apkTerrainId=29 apkKind=0 apkIsLand=false apkMoveCost=1；sourceTargetDist=2；moveTargetDist=1
- 引擎信息：-
